/** @file gameboardsingleplayer.cpp
  * @brief This file contains code for implementing the GameboardSinglePlayer class
  *
  * The functions defined in gameboardsingleplayer.cpp create the playable version of Battleship enumerated by the GameboardSinglePlayer
  * class. Broadly, provided functionality includes construction of the board, setting player/opponent ships, differentiated types of coordinates
...
#include "ui_gameboardsingleplayer.h"

//Code has been updated to utilize #include<random>
//Generates seed and linear congruential engine for future calls for random int values
unsigned int seed=std::chrono::system_clock::now().time_since_epoch().count();
std::default_random_engine generator(seed);

/**
 * @brief GameboardSinglePlayer::GameboardSinglePlayer
 * @param parent is the parent widget
 * Constructor for the GameboardSinglePlayer class creates widget with QLabel title/instructions, QCheckBox prompts for player to input ships
 * and QPushButton start/quit. In addition, constructor sets storage vectors to empty and gameplay int/bool values to 0 (default value).
 * Widget created by GameboardSinglePlayer constructor is eventually set as central widget in MainWindow
 */
GameboardSinglePlayer::GameboardSinglePlayer(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::GameboardSinglePlayer)
{
    ui->setupUi(this);
    ...
    int x=unif_x(generator);
    int y=unif_y(generator);
...
/**
 * @brief GameboardSinglePlayer::~GameboardSinglePlayer
 * Destructor for the GameboardSinglePlayer class. Qt objects automatically deal with memory management. Call delete on vectors containing pointers
 * to ensure no memory leaks
 */
GameboardSinglePlayer::~GameboardSinglePlayer()
{
...
/**
 * @brief GameboardSinglePlayer::updateLocation
 ...
 */
void GameboardSinglePlayer::updateLocation(int px, int py, int nx, int ny, std::vector<std::vector<QLabel*>>& grid, std::vector<std::vector<int>>& grid_type){
    grid[py][px]->clear();
    ...
    if(grid_type[py][px]==1){
        ...
        grid[py][px]->setStyleSheet("background-color:blue; font: bold 20px");
    }
...
/**
 * @brief GameboardSinglePlayer::set_location
 ...
 */
void GameboardSinglePlayer::set_location(){
...
/**
 * @brief GameboardSinglePlayer::keyPressEvent
 ...
 */
void GameboardSinglePlayer::keyPressEvent(QKeyEvent *event){
...
/**
 * @brief GameboardSinglePlayer::paintEvent
 ...
 * user
 */
void GameboardSinglePlayer::paintEvent(QPaintEvent *event){
    //if mode is 0 player is moving through their own board
    if(mode==0){
        int x=location->rx();
        int y=location->ry();
        ...
        if(player_type[y][x]==1){
            ...
            player[y][x]->setStyleSheet("background-color:blue; font: bold 20px; border: 6px solid");
        }
    ...
    if(mode==1){
        ...
        if(opponent_type[y][x]==1){
            ...
            opponent[y][x]->setStyleSheet("background-color:blue; font: bold 20px; border: 6px solid");
        }
...
/**
 * @brief GameboardSinglePlayer::showEvent
 * @param event is a pointer to the QShowEvent that displays the GameboardSinglePlayer object to the screen
 * showEvent function displays widget created by GameboardSinglePlayer object to screen. setFocus ensures that keyboard can be used (QPushButton/
 * QCheckBox are not the focus)
 */
void GameboardSinglePlayer::showEvent(QShowEvent *event) {
    this->activateWindow();
    this->setFocus();
...
/**
 * @brief GameboardSinglePlayer::isValid_aircraft_carrier
 ...
 */
bool GameboardSinglePlayer::isValid_aircraft_carrier(){
...
/**
 * @brief GameboardSinglePlayer::isValid_battleship
 ...
 * Return true is position is valid
 */
bool GameboardSinglePlayer::isValid_battleship(){
...
/**
 * @brief GameboardSinglePlayer::isValid_submarine
 ...
 * Return true is position is valid
 */
bool GameboardSinglePlayer::isValid_submarine(){
...
/**
 * @brief GameboardSinglePlayer::isValid_cruiser
 ...
 * Return true is position is valid
 */
bool GameboardSinglePlayer::isValid_cruiser(){
...
/**
 * @brief GameboardSinglePlayer::isValid_destroyer
 ...
 * Return true is position is valid
 */
bool GameboardSinglePlayer::isValid_destroyer(){
...
/**
 * @brief GameboardSinglePlayer::set_opponent_aircraft_carrier
 ...
 */
bool GameboardSinglePlayer::set_opponent_aircraft_carrier(){
    ...
    int orientation=unif_orientation(generator);
    ...
    if(orientation==0){
        ...
        int x=unif_x(generator);
        int y=unif_y(generator);
        int above_below=unif_above_below(generator);
...
/**
 * @brief GameboardSinglePlayer::set_opponent_battleship
 ...
 * if valid location
 */
bool GameboardSinglePlayer::set_opponent_battleship(){
...
/**
 * @brief GameboardSinglePlayer::set_opponent_submarine
 ...
 */
bool GameboardSinglePlayer::set_opponent_submarine(){
...
/**
 * @brief GameboardSinglePlayer::set_opponent_cruiser
 ...
 * Return true if valid location
 */
bool GameboardSinglePlayer::set_opponent_cruiser(){
...
/**
 * @brief GameboardSinglePlayer::set_opponent_destroyer
 ...
 */
bool GameboardSinglePlayer::set_opponent_destroyer(){
...
/**
 * @brief GameboardSinglePlayer::set_aircraft_carrier
 * Sets space type to 0 so that spacebar sets aircraft carrier position
 */
void GameboardSinglePlayer::set_aircraft_carrier(){
    space_type = 0;
    this->setFocus();
}

/**
 * @brief GameboardSinglePlayer::set_battleship
 * Sets space type to 1 so that spacebar sets battleship location
 */
void GameboardSinglePlayer::set_battleship(){
    space_type = 1;
    this->setFocus();
}

/**
 * @brief GameboardSinglePlayer::set_submarine
 * Sets space type to 2 so that spacebar sets submarine location
 */
void GameboardSinglePlayer::set_submarine(){
    space_type = 2;
    this->setFocus();
}

/**
 * @brief GameboardSinglePlayer::set_cruiser
 * Sets space type to 3 so that spacebar sets cruiser location
 */
void GameboardSinglePlayer::set_cruiser(){
    space_type = 3;
    this->setFocus();
}

/**
 * @brief GameboardSinglePlayer::set_destroyer
 * Sets space type to 4 so that spacebar sets destroyer location
 */
void GameboardSinglePlayer::set_destroyer(){
    space_type = 4;
    this->setFocus();
}

/**
 * @brief GameboardSinglePlayer::start_game
 ...
 * emptying player ship vectors. For latter scenario, displays QLabel informing player of need to reset their ships
 */
void GameboardSinglePlayer::start_game(){
    ...
    if(track_final){
        ...
        if(player_type[y][x]==1){
            player[y][x]->setText("X");
            player[y][x]->setStyleSheet("background-color:blue; font: bold 20px");
        }
...
/**
 * @brief GameboardSinglePlayer::set_opponent_board
 ...
 */
void GameboardSinglePlayer::set_opponent_board(){
...
/**
 * @brief GameboardSinglePlayer::clear_opponent_board
 * Iterates through opponent and opponent_type vectors of vectors to set to blue/0
 */
void GameboardSinglePlayer::clear_opponent_board(){
...
/**
 * @brief GameboardSinglePlayer::create_gray_box
 ...
 */
QLabel* GameboardSinglePlayer::create_gray_box(){
...
/**
 * @brief GameboardSinglePlayer::create_white_box
 ...
 */
QLabel* GameboardSinglePlayer::create_white_box(){
...
/**
 * @brief GameboardSinglePlayer::guess_location
 ...
 */
void GameboardSinglePlayer::guess_location(){
    int x=location->rx();
    int y=location->ry();
    ...
    if(opponent_type[y][x]==0){
        opponent_type[y][x]=1;
        opponent[y][x]->setText("X");
        opponent[y][x]->setStyleSheet("background-color:blue; font: bold 20px");
    }
...
/**
 * @brief GameboardSinglePlayer::opponent_turn
 ...
 * occurs by setting new random coordinates and running the function recursively if location type is 1 or 3.
 */
void GameboardSinglePlayer::opponent_turn(){
    std::uniform_int_distribution<int> unif_x(0,9);
    std::uniform_int_distribution<int> unif_y(0,9);
...
/**
 * @brief GameboardSinglePlayer::opponent_guess_up
 ...
 */
bool GameboardSinglePlayer::opponent_guess_up(){
    int x=opponent_location->rx();
    int y=opponent_location->ry();
    --y;
...
/**
 * @brief GameboardSinglePlayer::opponent_guess_down
 ...
 */
bool GameboardSinglePlayer::opponent_guess_down(){
...
/**
 * @brief GameboardSinglePlayer::opponent_guess_left
 ...
 */
bool GameboardSinglePlayer::opponent_guess_left(){
    int x=opponent_location->rx();
    int y=opponent_location->ry();
    --x;
    //decremented y must be at least 0 to stay in vector range
...
/**
 * @brief GameboardSinglePlayer::opponent_guess_right
 ...
 * incremented position and updates storage so that guesses may continue in direction
 */
bool GameboardSinglePlayer::opponent_guess_right(){
    int x=opponent_location_storage->rx();
    int y=opponent_location_storage->ry();
    ++x;
...
/**
 * @brief GameboardSinglePlayer::end_game
 * Ends application by calling close on parent widget of GameboardSinglePlayer
 */
void GameboardSinglePlayer::end_game(){
    parentWidget()->close();
}

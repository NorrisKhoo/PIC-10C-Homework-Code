...
/**	@class BinarySearchTree
	...
	with data of type T and comparison parameters of type CMP (default std::less). To enable move semantics, the move constructor was
	defined as BinarySearchTree utilizes pointers to locations in heap memory
*/
...
public:
	...
	BinarySearchTree<T, CMP>(BinarySearchTree<T, CMP>&& move_copy);
...
/**	Overloaded copy constructor creates a deep copy of a BinarySearchTree object. Operates by calling on the root the recursive node_copy
	...
	Prints statement "Copy made" to demonstrate when it is used. Part of implementing copy swap idiom
...
/**	Move constructor is overloaded because BinarySearchTree utilizes heap memory. Operates by assigning root pointer of rvalue reference
	to object being created and setting the original to nullptr. Prints statement "Move performed" to demonstrate when it is used
	@param move_copy is a rvalue reference to the BinarySearchTree object whose data is being given to the newly constructed object 
	(prevents an extra copy from being made)
*/
template<typename T, typename CMP>
BinarySearchTree<T, CMP>::BinarySearchTree(BinarySearchTree<T, CMP>&& move_copy)
{
	root = move_copy.root;
	move_copy.root = nullptr;
